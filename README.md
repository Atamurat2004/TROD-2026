...
Веб-сервис на **FastAPI** + **Uvicorn**, упакованный в **Docker**, с проверками **ruff**, **pytest** и **coverage** (порог **50%**), публикацией отчёта покрытия и сборкой/выгрузкой образа в **Docker Hub**.
...
| `.gitlab-ci.yml` | Основной пайплайн GitLab (MR + default branch) |
| `.github/workflows/ci.yml` | Тот же сценарий для GitHub Actions |
...
Пайплайн запускается при **открытом merge request** и при пуше в **ветку по умолчанию** (см. `workflow.rules` в `.gitlab-ci.yml`).
